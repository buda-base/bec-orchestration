...
def worker(
    ...
    if not s3_dest_bucket:
        logger.error(
            "Missing destination S3 bucket. Provide --s3-dest-bucket or set BEC_DEST_S3_BUCKET environment variable"
        )
        sys.exit(1)

    # Set model path in environment if provided (for ldv1/ocrv1 workers)
    if model_path:
        if "ocr" in job_name.lower():
            os.environ["BEC_OCR_MODEL_DIR"] = model_path
        else:
            os.environ["BEC_LD_MODEL_PATH"] = model_path
    elif "BEC_OCR_MODEL_DIR" not in os.environ and "ocr" in job_name.lower():
        logger.warning("BEC_OCR_MODEL_DIR not set. If this is an OCR job, provide --model-path or set env var.")
    elif "BEC_LD_MODEL_PATH" not in os.environ and "ld" in job_name.lower():
...
@click.option(
    ...
    "--force",
    is_flag=True,
    help="Force processing: bypass success.json check and claim task regardless of status",
)
...
def run_volume(
    ...
    try:
        ...
        if not force:
            console.print(
                "[yellow]Note:[/yellow] If this volume was already completed and you want to reprocess it, "
